...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let windowScn = UIWindow(windowScene: windowScene)
        var initialVC = UIViewController()
        let storyBoard = UIStoryboard(name: "MainApp", bundle: nil)
        if Auth.auth().currentUser != nil {
            initialVC = storyBoard.instantiateViewController(withIdentifier: "ChatVC")
           
        } else {
            initialVC = storyBoard.instantiateViewController(withIdentifier: "WelcomeVC")
            
        }
        windowScn.rootViewController = initialVC
        windowScn.makeKeyAndVisible()
        window = windowScn
